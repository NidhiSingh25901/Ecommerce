...
$ids =$_GET['id'];
echo $ids;
$showquery = "SELECT * FROM orderlists WHERE id=$ids AND customeremail='{$_SESSION['useremail']}' ";
...
$arrdata = mysqli_fetch_array($showdata);
 ...
 if(isset($_POST['order'])){ 
  ...
  $duplicatequery = "SELECT * FROM orderlists WHERE id = '$idupdate'  AND customeremail='{$_SESSION['useremail']}' ";
  
  $query = mysqli_query($con, $duplicatequery);
  
  $eduplicatecount = mysqli_num_rows($query);
    $query = "UPDATE orderlists SET productquantity='$productquantity' WHERE id=$idupdate" ;
    $iquery = mysqli_query($con,$query);
    
    if($iquery){
      ?> <script>alert("Product Quantity is updated");</script><?php
    }else{
      ?> <script>alert("There was an error in updating product quantity");</script><?php
    }
  }
...
<div class="head">
  ...
  <div class="menu-list">
    ...
    <div class="row products">
      ...
      <div class="col-md-3"> 
      <form method="POST" action="CafeCart.php">
        <div class="thumbnail">
...
<div class="addtocart">
  <p>To check your order, please go to <a href="cart.html">Cart</a></p>
</div>
</body>
